...
class ProjectDetailsRepository {
  ...
  Future<void> addComment(
    ...
    final commentsRef = projectRef.collection('comments');
    ...
    return _firestore.runTransaction((transaction) async {
      ...
      int reviewsCount =
          int.tryParse(data['reviews_count']?.toString() ?? '0') ?? 0;

      // === new rating by the user ===
      double userNewRating =
          double.tryParse(commentData['userRating']?.toString() ?? '0') ?? 0.0;

      // === calc the new rating ===
      double newAverageRating = currentRating;
      int newReviewsCount = reviewsCount;

      if (userNewRating > 0) {
        newReviewsCount = reviewsCount + 1;
        newAverageRating =
            ((currentRating * reviewsCount) + userNewRating) / newReviewsCount;
      }

      // === add the comment & update the project ===
      transaction.set(commentsRef.doc(), commentData); // add comment

      transaction.update(projectRef, {
        'rating': newAverageRating, // store the new rating
        'reviews_count': newReviewsCount, // update the number of reviews
      });
    });
  }

  // === update the rating ===
  Future<void> updateRating(String projectId, int stars) async {
...
// === check if the user saved the project before ===
  ...
  Future<void> updateProjectData(String projectId, Map<String, dynamic> data) async {
