...
class ProjectDetailsController extends ChangeNotifier {
  ...
  ProjectDetailsController({required this.project}) {
    ...
    numOfReviews = project.numOfReviews ?? 0;
  ...
  void _startProjectStream() {
    _projectSubscription = _repository.getProjectStream(project.id!).listen((
      ...
      if (snapshot.exists) {
        ...
        liveTotalFunds = (data['total_raised'] ?? 0.0).toDouble();
        liveInvestorsCount = (data['investors_count'] ?? 0);
        currentRating = (data['rating'] ?? 0.0).toDouble();
        numOfReviews = (data['reviews_count'] ?? 0);
        upvoteCount = (data['up_votes'] ?? 0);
  ...
  Future<void> addComment() async {
    ...
    try {
      await _repository.addComment(project.id!, {
        'name': UserProvider.instance.fullName,
        'job': 'Investor',
