...
class ContractGeneratorController {
  ...
  Future<void> generateAndSaveContract(ProjectModel project) async {
    try {
      ...
      investorsMap.forEach((key, value) {
        double equity = (value.amount / totalInvested) * 100;
        finalInvestors.add(
          ContractInvestor(
            userId: value.userId,
            name: value.name,
  ...
  String _generateLegalText(
    ...
    3. EQUITY & PROFIT DISTRIBUTION:
       - Net annual profits shall be distributed to investors pro-rata based on each investor's shareholding (as detailed in Schedule A).
    ...
    4. DONATIONS:
       Any funds contributed under the classification of "Donation" are considered non-refundable support. Such contributions grant no ownership rights, equity, or profit-sharing in the project and are recorded solely for transparency purposes.
